 ...
 *  세는 법(2026-09-17): 쓴 시간을 쓴 곳 전부에 적고, 어느 하나라도 60분을 넘으면 막는다.
  열쇠는 익명이면 IP + 브라우저 표시(쿠키), 로그인했으면 계정 + 브라우저 표시다.

  로그인해도 브라우저에 계속 적는 이유는 구멍 때문이다. 계정만 세면 익명으로 60분을
  다 쓰고 로그인해 60분을 더 쓸 수 있고, 로그인해서 다 쓰고 로그아웃해도 또 쓸 수 있다.
 ...
 *  있는지 밖에서는 알 수 없다 — 그게 NAT 다. IP 도 60분으로 재면 먼저 온 한 사람이
 ...
 */
export const IP_DAY_SECONDS = 3 * 60 * 60;

/** 하루에 서비스 전체가 쓸 수 있는 양. 10시간 = 실측 단가로 약 7,200원.
 *  개인 상한이 60분이니 정직하게 쓰는 사람 10명이 꽉 채워야 닿는 선이다. */
